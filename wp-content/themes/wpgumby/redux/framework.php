        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                     ...
                     *
                     * @pa
					 ram object $this ReduxFramework
                     */
...
// __construct()
            ...
            private function _internationalization() {
                ...
                if ( ReduxFramework::$_is_plugin ) {
                    load_plugin_textdomain( 'wpgumby', '', self::$_dir . 'languages/' . $locale . '.mo' );
                } else {
                    load_textdomain( 'wpgumby', self::$_dir . 'languages/' . $locale . '.mo' );
                }
                ...
                if ( ! empty ( $value ) ) {
                    ...
                    if ( $this->args['database'] === 'transient' ) {
                       ...
                       //     set_theme_mod( $k, $v );
						...
						$current_data= get_option( $this->args['opt_name']);
						foreach($value as $wpgumby_opt_key => $wpgumby_opt_val)
						{
							$def_value=$this->get_default_value($wpgumby_opt_key);
							
							if($wpgumby_opt_val==$def_value && !isset($current_data[$wpgumby_opt_key]))
							{
								unset($value[$wpgumby_opt_key]);
							}
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'wpgumby' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'wpgumby' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'wpgumby' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'wpgumby' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'wpgumby' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'wpgumby' ),
                        'action' => ''
                    ) );
                ...
                if ( ! current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'wpgumby' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'wpgumby' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                            ...
                            if (is_array($arr) && !empty($arr)) {
                                foreach($arr as $x => $y) {
                                    if (strpos(strtolower($y), 'redux') >= 0) {
                                        $msg = __('<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'wpgumby');
                    ...
                    if (!empty($this->args['share_icons'])) {
                        foreach($this->args['share_icons'] as $idx => $arr) {
                            if (is_array($arr) && !empty($arr)) {
                                foreach($arr as $x => $y) {
                                    if (strpos(strtolower($y), 'redux') >= 0) {
                                        $msg = __('<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'wpgumby');
