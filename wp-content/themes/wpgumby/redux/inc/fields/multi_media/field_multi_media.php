    ...
    class ReduxFramework_multi_media {
      ...
      // Set extension dir & url
        ...
        public function render() {
            ...
            $dev_tag        = '';
            $button_text    = isset($this->field['labels']['button']) ? $this->field['labels']['button'] : __('Add or Upload File(s)', 'wpgumby');
...
//            }
            ...
            if ($this->value && is_array($this->value)) {
                ...
                foreach($this->value as $id => $url){
                    ...
                    '<input
                        type="hidden"
                        value="' . $url . '"
                        name="' . $this->field['name'] .  $this->field['name_suffix'] . '[' . $id . ']' . '"
                        id="filelist-' . $id .'"
                        class=""
                    />';
                    ...
                    if ( $this->is_valid_img_ext( $url ) ) {
                        ...
                        '<li class="img_status">' .
                            ...
                            '<p class="redux_remove_wrapper"><a href="#" class="redux_remove_file_button">'. __( 'Remove Image', 'wpgumby' ) .'</a></p>
                            '. $id_input .'
                        </li>';
                    ...
                    // No image?  Output standard file info9
                        ...
                        $fileArr[] = 
                        '<li>' .
                            __( 'File: ', 'wpgumby' ) . ' <strong>' . $title . '</strong>&nbsp;&nbsp;&nbsp; (<a href="' . $url . '" target="_blank" rel="external">' . __( 'Download', 'wpgumby' ) .'</a> / <a href="#" class="redux_remove_file_button">'. __( 'Remove', 'wpgumby' ) .'</a>)
                            '. $id_input .'
                        </li>';
                    }
                }
	...
	private function is_valid_img_ext( $file ) {
        ...
        public function enqueue() {
            ...
            $upload_file    = isset($this->field['labels']['upload_file'])  ? $this->field['labels']['upload_file']     : __('Select File(s)', 'wpgumby');
            $remove_image   = isset($this->field['labels']['remove_image']) ? $this->field['labels']['remove_image']    : __('Remove Image', 'wpgumby');
            $remove_file    = isset($this->field['labels']['remove_file'])  ? $this->field['labels']['remove_file']     : __('Remove', 'wpgumby');
            $file_label     = isset($this->field['labels']['file'])         ? $this->field['labels']['file']            : __('File: ', 'wpgumby');
            $download_label = isset($this->field['labels']['download'])     ? $this->field['labels']['download']        : __('Download', 'wpgumby');
            $media_title    = isset($this->field['labels']['title'])        ? $this->field['labels']['title']           : 'Title';
            $dup_warn       = isset($this->field['labels']['duplicate'])    ? $this->field['labels']['duplicate']       : __('%s already exists in your file queue.','wpgumby');
            $max_warn       = isset($this->field['labels']['max_limit'])    ? $this->field['labels']['max_limit']       : __('Maximum upload limit of %s reached/exceeded.','wpgumby');
